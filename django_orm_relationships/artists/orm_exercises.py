...
def task_1_artist_exists():
    ...
    if allArtists.filter(artistic_name='Eric Clapton').exists():
        return True
    return False
...
def task_2_first_song_ordered():
    ...
    ordered = Song.objects.all().order_by('title')
...
def task_3_last_artist_ordered():
    ...
    ordered = Artist.objects.all().order_by('artistic_name')
    return ordered.last()
...
def task_4_artist_songs_contains():
    ...
    #first you want to return all the artists
    #Then you want to filter all the artists whose names havethe letter k
    #Then you want to return all the songs within this filtered list
...
def task_7_get_or_create_artist():
    ...
    obj, created = Artist.objects.get_or_create(artistic_name='Eric Clapton', defaults = {'first_name': 'Eric', 'last_name': 'Clapton', 'popularity': 95},)
    return created
...
def task_9_update_song_artist():
    ...
    artist = Artist.objects.create(artistic_name='Tiesto', popularity = 91)
