...
def task_4_artist_songs_contains(): #Question
    ...
    return Song.objects.filter(artist__artistic_name__icontains = 'K').select_related()
...
def task_7_get_or_create_artist():
    ...
    if Artist.objects.filter(artistic_name = 'Eric Clapton').exists():
        return False
    Artist.objects.get_or_create(artistic_name = 'Eric Clapton', defaults = {'popularity' : 85})
    return True
...
def task_9_update_song_artist():
    ...
    artist = Artist.objects.create(artistic_name = 'Eminem', first_name = 'Marshall', last_name = 'Mathers', popularity = 99)
