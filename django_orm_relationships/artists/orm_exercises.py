...
def task_1_artist_exists():
    ...
    art_exists = Artist.objects.filter(artistic_name__icontains="artistic_name").exists()
    if art_exists:
        return True
    else:
        return False
...
def task_4_artist_songs_contains():
    ...
    return Song.objects.all().filter(artist__artistic_name__icontains='k')
